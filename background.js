  ...
  try {
    chrome.runtime.sendMessage(msg, (resp) => {
      if (chrome.runtime.lastError) {
        // No receiver available (popup closed, etc) → ignore
      }
    });
  } catch (e) {
...
function awardPointsAndStreaks(record) {
  let pointsEarned = Math.max(1, Math.round(record.focusedSeconds / 60)); // 1 point per focused minute
...
function stopSession() {
  ...
  safeSendMessage({ type: "session_complete", summary: record });
...
function checkAchievements(record) {
  chrome.storage.local.get({ achievements: {}, sessions: [] }, (res) => {
    ...
    if (streakKeys.length >= 5 && !achievements.fiveDayStreak) {
